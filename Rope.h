...
public:
	...
	void concat_no_copy(Rope& r1, Rope& r2);
	...
	void updatesize(Node* ptr, int& newsize) const;
	Node* findindex(int& index) const;
	void splitnode(Node*& tmp, int i);

private:
	void print(std::ostream& out, Node* head, int depth) const;
	void destroy_tree(Node* ptr);
	Node* copy_tree(Node* rootptr, Node* parentptr);
